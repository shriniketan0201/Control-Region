...
void postAnalyzer::Loop(Long64_t maxEvents, int reportEvery)
{
  ...
  int nFilters, nHLT, nPhoCand, nWorstChIso, nGoodEle, nRecoil170, nDphiPhoRecoil, nDphiJetsMET, nMuVeto, nMETcut, nEleMETmT, nhem_veto;
  nFilters = nHLT = nPhoCand = nWorstChIso = nGoodEle = nRecoil170 = nDphiPhoRecoil = nDphiJetsMET = nMuVeto = nMETcut = nEleMETmT = nhem_veto=0;
					  ...
					  if(lepMET_MT < 160 && uncorrectedPhoEt/leptoMET < 1.4)
					    {
					      ...
					      if(hem_veto()==false){
                                                nhem_veto++;
                                                fillHistos(6,event_weight,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi);
...
Double_t postAnalyzer::fakerateeta(Double_t eta){
  Float_t weight = 1.0;
  /*
    outfile.txt:fake rate Sunil  1 =0.0505835
    outfile.txt:fake rate  Sunil  2 =0.0552314
    outfile.txt:fake rate  Sunil 2=0.0437457
    outfile.txt:fake rate  Sunil=0.0416802
    outfile.txt:fake rate  Sunil 5=0.0244503
  */

  if(fabs(eta) >= 1.566 && fabs(eta) < 2.5 ) weight = 0.0042;
