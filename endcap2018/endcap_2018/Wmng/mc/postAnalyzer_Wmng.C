	...
	if(HLTPho>>11&1 == 1)
	  {
		...
		//      if( TMath::Max( ( (*phoYuPFChWorstIso)[phoCand[0]]  - rho*EAchargedworst((*phoSCEta)[phoCand[0]]) ), 0.0) < 1.37 )
		{
		  ...
		  if(event_weight<0){
		    ...
		    cout<<"gen_weight = "<<genWeight<<endl;
		    cout<<"mytest"<<endl;
		  }
		  ...
		  if(mulist.size() == 1 && looseMus.size() == 1)
			...
			sfweight = sfcorrect->GetBinContent(sfcorrect->GetXaxis()->FindBin(fabs(muEta->at(mulist[0]))),sfcorrect->GetYaxis()->FindBin(muPt->at(mulist[0])));
		      ...
		      /*if(muweight<0.1 || sfweight<0.1){cout<<"muweight = "<<muweight<<", sfweight ="<<sfweight<<endl;
			cout<<"bincont : "<<mycorrect->GetXaxis()->FindBin(muPt->at(mulist[0]))<<", "<<mycorrect->GetYaxis()->FindBin(muEta->at(mulist[0]))<<endl;
			cout<<"@@@@@@ pt : "<<muPt->at(mulist[0])<<", @@@@@@@2eta "<<muEta->at(mulist[0])<<endl;
			}*/
		      //if(muPt->at(mulist[0])>= 120 || fabs(muEta->at(mulist[0]))>=2.4){muweight = sfweight = 1;}
		      cout<<"@@@@@@ pt : "<<muPt->at(mulist[0])<<", @@@@@@@2eta "<<muEta->at(mulist[0])<<"sf : "<<muweight<<" sf2 : "<<sfweight<<endl;
				  ...
				  if(looseEles.size() == 0)
				    {
				      ...
				      if(lepMET_MT < 160 && uncorrectedPhoEt/leptoMET_to_use < 1.4 && hem_veto()==false)
					{
